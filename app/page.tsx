...
export default function Home(){
 ...
 useEffect(()=>{supabase.auth.getSession().then(({data})=>{setSession(data.session);if(data.session)load();else setLoading(false)});const {data:l}=supabase.auth.onAuthStateChange((_e,s)=>{setSession(s);if(s)setTimeout(load,0)});return()=>l.subscription.unsubscribe()},[]);
 ...
 async function auth(e:React.FormEvent){
  ...
  if(mode==="register"){
   ...
   const r=await supabase.auth.signUp({email,password,options:{data:{full_name:name,whatsapp,cpf,birth_date:birth,marketing_consent:marketing,transactional_whatsapp:true,transactional_consent:true,terms_accepted:true}}});
 ...
 async function saveVehicle(e:React.FormEvent){
  e.preventDefault();setBusy(true);const r=await supabase.from("vehicles").insert({...vf,year:Number(vf.year),customer_id:customerId}).select().single();
 ...
 {view==="garage"&&<><div className="pageTitle"><span className="eyebrow dark">MINHA GARAGEM</span><h1>Seus veículos.</h1><p>O veículo é o centro do seu histórico na Qualific.Estetic.</p></div><div className="vehicleGrid">{vehicles.map(v=><div className="vehicleCard" key={v.id}><div className="vehicleVisual">🚘</div><div className="vehicleInfo"><span className="pill">{v.category}</span><h3>{v.make} {v.model}</h3><p>{v.year} · {v.color} · {v.plate}</p>{v.observations&&<small>{v.observations}</small>}<button className="linkBtn" onClick={()=>setView("history")}>Ver histórico</button></div></div>)}<button className="addVehicle" onClick={()=>setNewVehicle(true)}>＋ Adicionar veículo</button></div>{newVehicle&&<div className="modalBackdrop"><form className="modal panel" onSubmit={saveVehicle}><div className="panelTitle"><h3>Novo veículo</h3><button type="button" className="close" onClick={()=>setNewVehicle(false)}>×</button></div><div className="formGrid"><div className="field"><label>Marca</label><input required value={vf.make} onChange={e=>setVf({...vf,make:e.target.value})}/></div><div className="field"><label>Modelo</label><input required value={vf.model} onChange={e=>setVf({...vf,model:e.target.value})}/></div><div className="field"><label>Ano</label><input required type="number" value={vf.year} onChange={e=>setVf({...vf,year:e.target.value})}/></div><div className="field"><label>Cor</label><input required value={vf.color} onChange={e=>setVf({...vf,color:e.target.value})}/></div><div className="field"><label>Placa</label><input required value={vf.plate} onChange={e=>setVf({...vf,plate:e.target.value.toUpperCase()})}/></div><div className="field"><label>Categoria</label><select value={vf.category} onChange={e=>setVf({...vf,category:e.target.value})}><option value="HATCH">Hatch</option><option value="SEDAN">Sedan</option><option value="SUV">SUV</option><option value="PICKUP">Caminhonete</option><option value="OTHER">Outro</option></select></div></div><div className="field"><label>Observações permanentes</label><textarea value={vf.observations} onChange={e=>setVf({...vf,observations:e.target.value})}/></div><button className="btn primary full" disabled={busy}>Salvar veículo</button></form></div>}</>}
