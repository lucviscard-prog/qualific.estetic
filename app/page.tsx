...
export default function Home(){
  ...
  async function login(e:React.FormEvent){e.preventDefault();if(!supabase)return;setLoading(true);const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:window.location.origin}});setLoading(false);if(!error)setSent(true);}
  return <main>
    <section className="hero"><div className="container">
      ...
      <div className="heroGrid">
        ...
        <div className="card" id="acesso"><h3>Acesse sua garagem</h3><p>Entre sem senha. Enviaremos um link seguro para o seu WhatsApp ou e-mail.</p><form onSubmit={login}><div className="field"><label>E-mail</label><input required type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="voce@email.com"/></div><button className="btn primary" disabled={loading||!supabase}>{loading?"Enviando...":"Receber link de acesso"}</button>{sent&&<p style={{marginTop:14,color:"#17634e"}}>Link enviado. Verifique seu e-mail.</p>}{!supabase&&<p style={{marginTop:14,color:"#9a5b20"}}>Configure as variáveis do Supabase para ativar o acesso.</p>}</form></div>
      </div>
    </div></section>
